...
// Handle popup positioning and display
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "showPopup") {
    ...
    popup.style.left = pos.x + pos.width / 2 - popup.offsetWidth / 2 + "px";
    popup.style.top = pos.y - popup.offsetHeight - 10 + "px"; // 10px above
    popup.style.display = "block";

    // Store selected text for actions
    popup.dataset.selectedText = message.text;
  }
});
...
document
  ...
  .addEventListener("click", async (e) => {
    ...
    const selectedText =
      document.getElementById("floating-popup").dataset.selectedText;
    let response;
